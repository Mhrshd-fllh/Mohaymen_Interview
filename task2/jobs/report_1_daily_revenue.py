...
logger = logging.getLogger("report_1_daily_revenue")
...
def build_daily_revenue_stream(
    ...
    logger.info("Configuring 1-day tumbling window aggregaion on 'RECORD_DATE'...")

    daily_aggregated_df = (
        watermarked_df.groupBy(window(col("RECORD_DATE"), "1 day"))
        .agg(
    ...
    projected_df = daily_aggregated_df.select(
        date_format(col("window.start"), "yyyy/MM/dd").alias("RECORD_DATE"),
        col("revenue"),
        col("Record_Count")
    )
...
def run_report_1(
    ...
    spark = get_spark_session(app_name="Report_1_Daily_Total_Revenue")
    ...
    report_stream = build_daily_revenue_stream(raw_stream, watermark_delay=watermark_delay)
    ...
    if sink_mode == "console":
        ...
        def write_micro_batch(batch_df: DataFrame, batch_id: int):
            ...
            if batch_count > 0:
                ...
                batch_df.coalesce(1).write.mode("overwrite").option("header", "true").csv(batch_target_path)
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Spark Structured Streaming — Report 1: Daily Total Revenue")
    parser.add_argument(
        "--input-path",
        type=str,
    ...
    parser.add_argument(
        "--output-path",
        type=str,
        default="s3a://reports-bucket/reports/report_1_daily_revenue/",
        help="S3A or local destination path for output CSV reports.",
    )
    parser.add_argument(
        "--checkpoint-path",
        type=str,
        default="s3a://reports-bucket/checkpoints/report_1_daily_revenue/",
