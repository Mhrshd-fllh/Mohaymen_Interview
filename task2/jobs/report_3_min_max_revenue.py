...
logger = logging.getLogger("report_3_min_max_revenue")
...
def build_min_max_revenue_stream(
    ...
    logger.info(
        "Configuring %s tumbling window multi-metric aggregation grouped by (window, paytype)...",
        window_duration,
    )
    windowed_aggregated_df = (
        ...
        .agg(
            spark_min("revenue").cast(DecimalType(18, 2)).alias("min_revenue"),
            spark_max("revenue").cast(DecimalType(18, 2)).alias("max_revenue"),
            spark_sum("revenue").cast(DecimalType(18, 2)).alias("total_revenue"),
...
def run_report_3(
    ...
    spark = get_spark_session(app_name="Report_3_Min_Max_Revenue_per_PayType")
    ...
    report_stream = build_min_max_revenue_stream(
        raw_stream,
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Spark Structured Streaming — Report 3: Min & Max Revenue per PayType")
    parser.add_argument(
        "--input-path",
        type=str,
    ...
    parser.add_argument(
        "--output-path",
        type=str,
        default="s3a://reports-bucket/reports/report_3_min_max_revenue/",
        help="S3A or local destination path for output CSV reports.",
    )
    parser.add_argument(
        "--checkpoint-path",
        type=str,
        default="s3a://reports-bucket/checkpoints/report_3_min_max_revenue/",
